...
select target_name, prank_description, evilness_score
  from
(select prank_id, target_name, prank_description, evilness_score, row_number() over (partition by target_name order by evilness_score desc, created_at desc) as rown
from grinch_prank_ideas) m 
where rown = 1
